...
public class BancosDaoJDBC implements BancosDao {
	...
	public Bancos findById(Integer id) {
		...
		try {
			...
			if (rs.next()) {
				...
				Bancos obj = instantiateBancos(rs, dep);
				return obj;
			}
			return null;
		} catch (SQLException e) {
	...
	private Bancos instantiateBancos(ResultSet rs, Department dep) throws SQLException {
	...
	@Override
	public List<Bancos> findByDepartment(Department department) {
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			st = conn.prepareStatement(
